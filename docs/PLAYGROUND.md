...
入口：[在线运行](https://rudykon.github.io/PersonaGuard/demo/)，或主页的“运行案例”按钮。
...
- 条件只存在当前页面内存中，不上传，也不保存到浏览器持久存储；语言由当前中英文页面地址决定。
...
| `website/overrides/content/demo.html`、`website/javascripts/playground.js`、`website/stylesheets/personaguard.css` | 双语表单、结果解释与移动端展示 |
...
按 MkDocs 输出的本地地址打开 `demo/`。网站环境安装见 [文档导航](README.md)；旧 `playground.html` 地址保留跳转，案例查询参数保持不变。

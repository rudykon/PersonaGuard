...
The canonical input is ``results/revision6_source.json``.  This script owns the
...
def load_source() -> dict[str, object]:
    ...
    if value.get("schema_version") != "revision6-publication-source-v1":
        raise RuntimeError("results/revision6_source.json has the wrong schema")
...
def macros(source: dict[str, object], *, chinese: bool = False) -> str:
    ...
    header = (
        [
            ...
            "% 请勿编辑：results/revision6_source.json 是唯一数值来源。",
        ]
        if chinese
        else [
            "% Generated by scripts/generate_revision6_publication.py.",
            "% Do not edit: results/revision6_source.json is the sole numerical source.",
        ]
    )
...
def readme_block(source: dict[str, object], *, paper: bool) -> str:
    ...
    location = "`results/revision6_source.json`" if not paper else "`../results/revision6_source.json`"
...
def expected_files(source: dict[str, object]) -> dict[Path, str]:
    outputs = {
        ...
        ROOT / "results" / "statistical_analysis_registry.json": json.dumps(
